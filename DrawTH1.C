...
void DrawTH1(OBS_ID obsID=oVtxXYsliding, Int_t nbin=50, Double_t bmin=0, Double_t bmax=5000, Bool_t drawstack = false){
  ...
  auto hs = new THStack("hs",Form("%s;%s;%s",xlabel.Data(),xlabel.Data(),ScalePlots?"Events x Weight":"Events"));;
  hs->Add(h_munuqq);
  hs->Add(h_zmumu);
  hs->Add(h_ztautau);
  hs->Add(h_zuds);
