...
void TREESetBranch(){
  ...
  TREE->SetBranchAddress("NtracksP",&oNtracksP);
  TREE->SetBranchAddress("NtracksS",&oNtracksS);
  ...
  TREE->SetBranchAddress("Vtx_xP",&oVtx_xP);
  TREE->SetBranchAddress("Vtx_yP",&oVtx_yP);
  TREE->SetBranchAddress("Vtx_zP",&oVtx_zP);
  TREE->SetBranchAddress("Vtx_chi2P",&oVtx_chi2P);
  TREE->SetBranchAddress("Vtx_ntrkP",&oVtx_ntrkP);
  TREE->SetBranchAddress("Vtx_xS",&oVtx_xS);
  TREE->SetBranchAddress("Vtx_yS",&oVtx_yS);
  TREE->SetBranchAddress("Vtx_zS",&oVtx_zS);
  TREE->SetBranchAddress("Vtx_chi2S",&oVtx_chi2S);
  TREE->SetBranchAddress("Vtx_ntrkS",&oVtx_ntrkS);
}
...
// THIS IS TO IMPLEMENT CUTS ON THE FLY (MAINLY FOR TEST). IT IS CALLED BY ALL THE NEXT ONES. BUT RETURNS TRUE IF THE ANALYSIS OPT DOESN'T CONTAIN "extra"
Bool_t EXTRA(TString analysis_opt){

  if (!analysis_opt.Contains("extra")){
    return true;
  }

  bool sel1 = (oNtracks - oNtracksP) < 5;
  bool sel2 = TMath::Log10(oVtx_chi2P) < 1;

  if (!sel1) return false;
  if (!sel2) return false;

  bool selv = TMath::Sqrt(oVtx_xP*oVtx_xP + oVtx_yP*oVtx_yP) < 0.5;
...
Bool_t SELECTION_STRING_KINE(int jalg, TString analysis_opt){ // at the moment it is implemented only for the default "anymass1L2M"
  ...
  if (!EXTRA(analysis_opt)) return false;
