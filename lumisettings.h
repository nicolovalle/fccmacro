...
Double_t Nnocut(TString opt, TString lifetime="n/a"){

  if (lifetime != "n/a") return 10000.;
...
Double_t xsec(TString opt, TString HNMass = "50", TString lifetime="n/a"){
  ...
  else if (lifetime == "n/a") return xs_vs_m[HNMass];
...
Double_t Weight(TString opt, TString HNMass="specify only for signal", TString lifetime="n/a", Double_t custom_weight = 1.){
  ...
  return xsec(opt,HNMass,lifetime)*LUMI/(Nnocut(opt,lifetime) * custom_weight);
